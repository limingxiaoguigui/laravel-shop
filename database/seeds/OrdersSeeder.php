    ...
    public function run()
    {
        ...
        foreach ($orders as $order) {
            ...
            $item = factory(OrderItem::class, random_int(1, 3))->create([
                ...
                'reviewed_at' => $order->reviewed ? $faker->dateTimeBetween($order->paid_at) : null, //评价时间不能早于不支付时间
            ]);
            //计算总价
            $total = $item->sum(function (OrderItem $item) {
        ...
        $products->unique('id')->each(function (Product $product) {
            ...
            $product->update([
                'rating' => $result->rating ?: 5, //如果商品没有评分则默认5分
